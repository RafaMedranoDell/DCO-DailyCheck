...
def create_DCI(dcocfg, dcorpt):
    ...
    for instance in dcocfg.instances(system):
        ...
        if not healthSystemStatus.data.empty:
            ...
            dcorpt.add_table("Protection", "PowerProtect DataManager", f"Instance {instance}", "Health Status", healthSystemStatus, tableset="ts1")
        ...
        if not healthByCategory.data.empty:
            dcorpt.add_table("Protection", "PowerProtect DataManager", f"Instance {instance}", "Health By Category", healthByCategory, tableset="ts1")

        jobgroupSummary = DCOreport.csv_to_styleddf(system, instance,  "jobgroupSummary", dcocfg)
        if not jobgroupSummary.data.empty:
            dcorpt.add_table("Protection", "PowerProtect DataManager", f"Instance {instance}", "Job Group Summary", jobgroupSummary, tableset="ts2")

        jobgroupRate = DCOreport.csv_to_styleddf(system, instance,  "jobgroupRate", dcocfg)
        if not jobgroupRate.data.empty:
            dcorpt.add_table("Protection", "PowerProtect DataManager", f"Instance {instance}", "Job Group Rate", jobgroupRate, tableset="ts2")

        healthEvents = DCOreport.csv_to_styleddf(system, instance,  "healthEvents", dcocfg)
        if not healthEvents.data.empty:
            dcorpt.add_table("Protection", "PowerProtect DataManager", f"Instance {instance}", "Health Events", healthEvents, tableset="ts3")



        storageSystems = DCOreport.csv_to_styleddf(system, instance,  "storageSystems", dcocfg)
        if not storageSystems.data.empty:
            dcorpt.add_table("Protection", "PowerProtect DataManager", f"Instance {instance}", "Storage Systems", storageSystems, tableset="ts5")
